...
> **Status:** v0.6.0 (August 27, 2026) — the first public release: the customs declaration (what leaves your machine is shown before any remote send and receipted after, ADR-26); [FreeToken](https://github.com/FlashML-org/FreeToken) as a recognized local engine with a frontier sparse-MoE tier on the starter shelf, GPU-aware sizing, and a model's reasoning shown live in chat (never stored); the starter shelf and the cognition role; the open-weights rails; cognition sovereignty and the public [Sovereignty Ledger](docs/SOVEREIGNTY.md); Life Import rail #1 (email); the August 2026 [security audit](docs/SECURITY_AUDIT_2026-08.md) with every recommendation resolved; a multi-arch container image and a Linux arm64 binary; and the public site at [mysovereign.ai](https://mysovereign.ai). Before it, v0.5.0 brought portability both ways — memory provenance, checksummed (and optionally encrypted) export archives with a [documented open format](docs/EXPORT_FORMAT.md), the Personal Context Portfolio, and opt-in memory distillation — on top of the command center UI, source-aware chat, Fine-Tuning Studio, BYOC rails, chat-history import, and single binaries.
...
- **The customs declaration** — before any message goes to a remote provider, the web UI shows exactly what would leave: the system prompt, the memories placed in it, the retrieved excerpts, the prior messages, and your message, each expandable, with sizes and the host. Send, cancel, or stop asking for that provider (revocable in Settings). Local endpoints never ask — nothing leaves — and every remote answer carries a receipt ("left the machine · 6.2 KB → api.openai.com"). No classifier decides what is sensitive: you see the bytes.
...
- **Own the brain** — Ollama models (the starter shelf's August 2026 picks include Qwen3.8-27B — dense, Apache-2.0, 262K context, vision — which fits a 32 GB machine at Q4, and NVIDIA's Nemotron 3.5 Lightning, the first frontier-tier sparse model Ollama pulls directly), [FreeToken](https://github.com/FlashML-org/FreeToken) for frontier-class sparse MoE models on one gaming GPU (20B–120B total, 3–5B active per token), OpenAI-compatible servers (vLLM, llama.cpp, LM Studio, Groq…), or BYO-key Anthropic; switchable per persona. Model Studio saves portable recipes and builds custom named artifacts on the Ollama endpoint you control.
...
The covenant is enforced by license text, not promised: your exit is a date you can compute, not our goodwill. Nothing an individual needs to own all twelve layers is ever paywalled, rate-limited, or feature-stripped; the commercial path works by addition (a managed edition, organization features, commercial grants — see the [licensing policy](docs/LICENSING.md)), never by clawing back what's free. The code is yours to run; the [name is not](TRADEMARKS.md). Contributions ship under the [DCO](CONTRIBUTING.md) — no CLA to sign. The thin-client integrations (VS Code, JetBrains, browser) remain plain MIT.
