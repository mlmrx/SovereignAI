...
test('release integration source versions align with SovereignAI v0.3', () => {
  assert.equal(JSON.parse(read('integrations/browser/manifest.json')).version, '0.3.0');
  assert.equal(JSON.parse(read('integrations/vscode/package.json')).version, '0.3.0');
  assert.match(read('integrations/jetbrains/build.gradle.kts'), /version = "0\.3\.0"/);
  assert.match(read('integrations/chatgpt/openapi.yaml'), /version: 0\.3\.0/);
});
