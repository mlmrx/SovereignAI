...
function existingContainerRecord(home, overrides = {}) {
  return openRegistry(home).save({
    name: 'main',
    createdAt: new Date().toISOString(),
    computeStyle: 'container',
    provider: { name: 'runpod', instanceId: 'pod-xyz', gpuTypeId: 'NVIDIA GeForce RTX 4090', image: 'ghcr.io/x/sovereignai:v1' },
    host: '198.51.100.7',
    port: 41234,
    tokenSha256: 'deadbeef',
    app: { version: '0.4.0', setupComplete: true },
    status: 'running',
    ...overrides,
  });
}

// All of these stay entirely offline: unknown-provider/missing-flag errors
// are caught before any network code runs, and every plan-without-apply
// case relies on the same non-TTY confirm()-returns-false behavior the
// existing "plan-without-apply" test above already exercises for rail #1.
test('byoc gpu CLI: help, guardrails, and plan-without-apply for both provider styles', (t) => {
  const home = makeTemp(t, 'byoc-gpu-cli');

  const help = runCli(['byoc', 'gpu', 'help'], { home });
  assert.equal(help.status, 0, help.stderr);
  assert.match(help.stdout, /UNVERIFIED against live provider infrastructure/);
  assert.match(help.stdout, /runpod, vastai/);

  const bareGpu = runCli(['byoc', 'gpu'], { home });
  assert.equal(bareGpu.status, 0, 'no subcommand defaults to help, same convention as bare "byoc"');
  assert.match(bareGpu.stdout, /UNVERIFIED/);

  const unknownAction = runCli(['byoc', 'gpu', 'frob'], { home });
  assert.equal(unknownAction.status, 1);
  assert.match(unknownAction.stderr, /Unknown byoc gpu action/);

  const listNoProvider = runCli(['byoc', 'gpu', 'list'], { home });
  assert.equal(listNoProvider.status, 1);
  assert.match(listNoProvider.stderr, /gpu list requires a provider/);

  const listUnknownProvider = runCli(['byoc', 'gpu', 'list', 'nope'], { home });
  assert.equal(listUnknownProvider.status, 1);
  assert.match(listUnknownProvider.stderr, /Unknown GPU provider "nope"/);

  const listNoKey = runCli(['byoc', 'gpu', 'list', 'runpod'], { home });
  assert.equal(listNoKey.status, 1);
  assert.match(listNoKey.stderr, /gpu list needs an API key/);
  assert.match(listNoKey.stderr, /RUNPOD_API_KEY/);

  const deployNoProvider = runCli(['byoc', 'gpu', 'deploy'], { home });
  assert.equal(deployNoProvider.status, 1);
  assert.match(deployNoProvider.stderr, /gpu deploy requires a provider/);

  const deployNoGpuType = runCli(['byoc', 'gpu', 'deploy', 'runpod', '--api-key', 'k'], { home });
  assert.equal(deployNoGpuType.status, 1);
  assert.match(deployNoGpuType.stderr, /requires --gpu-type/);

  const deployNoImage = runCli(['byoc', 'gpu', 'deploy', 'runpod', '--gpu-type', 'x', '--api-key', 'k'], { home });
  assert.equal(deployNoImage.status, 1);
  assert.match(deployNoImage.stderr, /needs a pullable image/);

  // Container-style plan, no --yes: prints the plan (including the trust-model
  // disclosure) and provisions nothing.
  const runpodPlan = runCli(
    ['byoc', 'gpu', 'deploy', 'runpod', '--gpu-type', 'NVIDIA GeForce RTX 4090', '--api-key', 'k', '--image', 'ghcr.io/x/sovereignai:v1', '--name', 'planned'],
    { home }
  );
  assert.equal(runpodPlan.status, 0, runpodPlan.stderr);
  assert.match(runpodPlan.stdout, /GPU deployment plan/);
  assert.match(runpodPlan.stdout, /container-style/);
  assert.match(runpodPlan.stdout, /generated by THIS CLI/);
  assert.match(runpodPlan.stdout, /UNVERIFIED against live RunPod infrastructure/);
  assert.match(runpodPlan.stdout, /Nothing was provisioned/);
  assert.equal(fs.existsSync(path.join(home, 'byoc', 'instances.json')), false);

  // VM-style (lambda) plan, no --image: falls back to source-build, same as rail #1.
  const lambdaPlan = runCli(['byoc', 'gpu', 'deploy', 'lambda', '--gpu-type', 'gpu_1x_a10', '--api-key', 'k', '--name', 'planned'], { home });
  assert.equal(lambdaPlan.status, 0, lambdaPlan.stderr);
  assert.match(lambdaPlan.stdout, /vm-style/);
  assert.match(lambdaPlan.stdout, /build on the host from your committed source/);
  assert.match(lambdaPlan.stdout, /generated ON the host, same trust model as rail #1/);
  assert.match(lambdaPlan.stdout, /Nothing was provisioned/);
  assert.equal(fs.existsSync(path.join(home, 'byoc', 'instances.json')), false);

  // Existing lifecycle actions reject a container-style record with a clear
  // message instead of crashing on the missing record.ssh.
  existingContainerRecord(home);
  for (const action of ['upgrade', 'suspend', 'resume', 'export']) {
    const result = runCli(['byoc', action, 'main'], { home });
    assert.equal(result.status, 1, action);
    assert.match(result.stderr, /no SSH access, so this action isn't supported/, action);
  }

  // list handles a container-style record (no record.ssh) without crashing.
  const list = runCli(['byoc', 'list'], { home });
  assert.equal(list.status, 0, list.stderr);
  assert.match(list.stdout, /runpod instance pod-xyz @ 198\.51\.100\.7:41234/);

  // destroy on a container-style record, without an API key: warns and keeps
  // the local record (rather than pretending it terminated the instance) — no
  // network call happens because the missing-key check runs before fetch.
  const destroyNoKey = runCli(['byoc', 'destroy', 'main', '--yes'], { home });
  assert.equal(destroyNoKey.status, 1);
  assert.match(destroyNoKey.stdout, /WARNING: could not terminate/);
  assert.match(destroyNoKey.stdout, /RUNPOD_API_KEY/);
  assert.notEqual(openRegistry(home).get('main'), null, 'the record survives so the operator can retry with a key');
});
